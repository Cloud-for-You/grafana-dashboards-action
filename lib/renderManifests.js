...
function init(workingDir, configFile, tmpDir) {
  ...
  try {
    ...
    if (valid) {
      ...
      console.log('Config file in path ' + path.dirname(configFile) + ' has not valid format');
...
function renderManifest(name, data, config, result = Object) {
  const grafanaFolder = config.grafanaFolder;
  const instanceSelector = config.instanceSelector;
  ...
  instanceSelector.forEach(function(instanceItem) {
    k8s_manifest.spec.instanceSelector.matchLabels[instanceItem.key] = instanceItem.value;
  });
